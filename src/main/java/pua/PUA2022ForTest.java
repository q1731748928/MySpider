...
public class PUA2022ForTest {
    ...
    public static void main(String[] args) throws IOException, ExecutionException, InterruptedException {
        ...
        ReadTxtFileUtil.toNullFile("" + "\n\n");
        getName(COOKIESTR);
        //Desktop.getDesktop().open(new File("G:\\PUA2022\\ALL\\pua.txt"));
    }
...
//            System.out.println("url = " + url);
            if (!url.toLowerCase().endsWith(".zip") && !url.toLowerCase().endsWith(".mov") && !url.toLowerCase().endsWith(".pdf") && !url.toLowerCase().endsWith(".jpg") && !url.toLowerCase().endsWith(".jpeg") && !url.endsWith(".m4a") && !url.toLowerCase().endsWith(".png") && !url.toLowerCase().endsWith(".mp4") && !url.toLowerCase().endsWith(".mp3") && !url.toLowerCase().endsWith(".doc") && !url.toLowerCase().endsWith(".docx") && !url.toLowerCase().endsWith(".avi") && !url.toLowerCase().endsWith(".pptx")
                    && !url.toLowerCase().endsWith(".ppt") && !url.toLowerCase().endsWith(".txt") && !url.toLowerCase().endsWith(".rar") && !url.toLowerCase().endsWith(".mkv") && !url.toLowerCase().endsWith(".ts") && !url.toLowerCase().endsWith(".flv") && !url.toLowerCase().endsWith(".html")) {
        ...
        if (!url.toLowerCase().endsWith(".zip") && !url.toLowerCase().endsWith(".mov") && !url.toLowerCase().endsWith(".pdf") && !url.toLowerCase().endsWith(".jpg") && !url.toLowerCase().endsWith(".jpeg") && !url.endsWith(".m4a") && !url.toLowerCase().endsWith(".png") && !url.toLowerCase().endsWith(".mp4") && !url.toLowerCase().endsWith(".mp3") && !url.toLowerCase().endsWith(".doc") && !url.toLowerCase().endsWith(".docx") && !url.toLowerCase().endsWith(".avi") && !url.toLowerCase().endsWith(".pptx")
            ...
            if (json.contains("data")) {
                ...
                for (int i = 0; i < objects.length(); i++) {
                    ...
                    String name = jsonObject1.getString("name");
                    String path = jsonObject1.getString("path");
//                    ReadTxtFileUtil.toFile(path + "\n", downloadPath + "\\" +"puaDetail.txt", true);
//                    ReadTxtFileUtil.toFile(name + "\n\n", "G:\\PUA2022\\nameTmp.txt", true);
                    url += URLEncoder.encode(path + "/" + name, "UTF-8");
                    if (url.contains("%2F%2F")) {
                        url = url.replace("%2F%2F", "%2F");
                    }
                    if (!url.toLowerCase().endsWith(".mov") && !url.toLowerCase().endsWith(".pdf") && !url.toLowerCase().endsWith(".jpg") && !url.toLowerCase().endsWith(".jpeg") && !url.endsWith(".m4a") && !url.toLowerCase().endsWith(".png") && !url.toLowerCase().endsWith(".mp4") && !url.toLowerCase().endsWith(".mp3") && !url.toLowerCase().endsWith(".doc") && !url.toLowerCase().endsWith(".docx") && !url.toLowerCase().endsWith(".avi") && !url.toLowerCase().endsWith(".pptx")
                            && !url.toLowerCase().endsWith(".ppt") && !url.toLowerCase().endsWith(".txt") && !url.toLowerCase().endsWith(".rar") && !url.toLowerCase().endsWith(".mkv") && !url.toLowerCase().endsWith(".ts") && !url.toLowerCase().endsWith(".flv") && !url.toLowerCase().endsWith(".html")) {
                        getNName(url, COOKIESTR);
                    } else {
                        String baseUrl = " https://pan.pua333.cn/api/v3/share/download/y2sk?path=";
...
//                ReadTxtFileUtil.toFile(decode + "\n\n", downloadPath + "\\" + "notDown.txt", true);
